...
# Harmonize to 2023 municipality borders
...
## DF : in year 2025, was the muni ever part of a merger?

cw_info_ever_merged_ags_25 <- cw %>%
...
streitholz <- data.frame(
  ...
  ags_25 = "16061049", ags_name_25 = "Hohes Kreuz",
...
df_harm <- votes |>
  # Rename ags
  left_join_check_obs(area_pop, by = c("ags_25", "election_year")) |>
  rename(ags = ags_25) |>
  mutate(ags = pad_zero_conditional(ags, 7)) |>
...
df_harm <- df_harm |>
  ...
  dplyr::select(-c(ags_name, ags_name_25, emp_cw, employees, year_cw, id)) |>
  # arrange
  arrange(ags, election_year)
...
df_harm <- read_rds("data/federal_elections/municipality_level/final/federal_muni_harm_25.rds") |>
  arrange(ags, election_year)
