...
cw <- gerda_add_geckler_backmap(cw, "ags_21",
...
df_cw <- df_cw_naive |>
  mutate(
    ...
    crosswalk_year = ifelse(!is.na(ags_21), election_year, NA_real_),
    mapping_method = ifelse(!is.na(ags_21), "exact_year", "unmatched")
  )
...
if (nrow(df_unmatched) > 0) {
  df_unmatched <- df_unmatched |>
    ...
    mutate(crosswalk_year = ifelse(!is.na(ags_21), year_cw, NA_real_),
           mapping_method = ifelse(!is.na(ags_21), "previous_year", "unmatched"))
}
...
if (nrow(still_unmatched) > 0) {
  ...
  best_cw_year <- gerda_nearest_crosswalk_year(unmatched_keys, cw, "ags_21", "state_harm_21")
  ...
  still_unmatched <- still_unmatched |>
    ...
    mutate(crosswalk_year = cw_year,
           mapping_method = ifelse(!is.na(ags_21), "nearest_year", "unmatched")) |>
    select(-cw_year)
...
gerda_audit_totals(
  df, votes |> mutate(state = substr(ags_21, 1, 2)),
  "election_year",
  c("eligible_voters", "number_voters", "valid_votes", "invalid_votes", party_vars),
  "state_harm_21")
votes <- votes |>
...
write_rds(df_final, "data/state_elections/final/state_harm_21.rds", compress = "gz")
