...
- ST (`00_st_stala_parse.py`) is a parse of the Statistisches Landesamt Sachsen-Anhalt Dezernat 13 "Datensatzbeschreibung Bürgermeisterwahlen" extract (`bmbm.csv`, Stand 26.02.2025). Covers **all 218 currently-serving mayors** — one row per Gemeinde with the most recent Bürgermeisterwahl in the 7-year Amtszeit window (2019-11-10 through 2026-02-15), all up-to-15 candidate slots, party affiliations (formal Wahlvorschlag), HW+SW turnout, and the row-level Elected record. The Landeswahlleiter portal scrape (`00_st_scrape.py`) is retained as a **fallback** for the 5 post-2026-02-15 elections not yet in the StaLA extract, and for the single missing Wöhling candidate at Genthin 2024 (corrupted B08/B09 in the StaLA CSV). Hybrid merge rule in R (01/01b): StaLA rows first; portal `(ags, election_date)` pairs absent from StaLA appended; portal candidates in shared elections appended only if their (last_name, first_name, votes) match NONE of StaLA — catches Genthin Wöhling while dropping Wiedecke/Nase duplicates.
