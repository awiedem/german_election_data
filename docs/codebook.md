...
| `flag_partial_coverage`         | integer | County-council `_cty` output only. 1 where the source covers less than 99 % of the 2021 county's population, so the row's **counts** describe only part of the county — turnout and vote shares remain valid for the part that voted. `NA` means not assessed (the row was aggregated from municipality-level sources). Currently set on the six 2021 Saxon counties that overlap the four 1994 Kreise whose Kreistagswahl the Sächsisches Verfassungsgericht annulled. |
