...
**Files:** `county_elec_unharm` (68,937 x 411),
`county_elec_harm_21_muni` (41,941 x 422), `county_elec_harm_21_cty`
(2,303 x 419), `county_council_seats` (7,200 x 22) in
`data/county_elections/final/`.

Shared blocks apply, with 400 party columns. Results are reported at
...
Brandenburg has a parsing block but currently contributes no rows (a
tibble name-mangling failure in its Stage 1 parser).
